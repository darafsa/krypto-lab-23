...
def calculate_partitionated_coincidence_index(data: str):
	...
	frequency_sum = np.sum([letter_frequency * (letter_frequency - 1)
...
def calculate_key(data: str, most_common_char: str, key_length: int):
	filtered_data = "".join(char for char in data if char in alphabet)
...
key_length = calculate_key_length(input_data, alphabet, 0.07)
key = calculate_key(input_data, 'E', key_length)
